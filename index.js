...
const mainInterval = setInterval(function () {
  if (block || isPaused) {
    ...
    if (a) {
      ...
      let capturedOut = [], capturedSent = false, capturedScanned = 0;
      ...
      global.WebSocket = global.window.WebSocket = new Proxy(WebSocket, {
        construct: function (a, b, c) {
          ...
          d.send = new Proxy(d.send, { apply: function (f, g, h) {
            // Capture the first outbound handshake packets (hello 'k',
            // fingerprint 'T') so the protocol-only client can replay
            // genuine packets instead of sanitized synthetic ones.
            if (process.env.ARRAS_CAPTURE_SOCKET_URL_ONLY === '1' && capturedScanned < 300) {
              try {
                const arg = h[0];
                let hex = null;
                if (typeof arg === 'string') { hex = Buffer.from(arg).toString('hex'); }
                else if (arg instanceof ArrayBuffer) { hex = Buffer.from(arg).toString('hex'); }
                else if (ArrayBuffer.isView(arg)) {
                  // Views (e.g. Uint8Array over wasm memory) must be sliced
                  // at byteOffset/byteLength — the backing buffer is huge.
                  hex = Buffer.from(arg.buffer, arg.byteOffset, arg.byteLength).toString('hex');
                }
                if (hex) {
                  capturedScanned++;
                  // Debug: record every raw outbound send for diagnosis.
                  if (capturedScanned <= 25) {
                    try {
                      fs.appendFileSync(
                        path.join(__dirname, `vm-out-debug-${process.pid}.log`),
                        `${capturedScanned} len=${hex.length / 2} head=${hex.slice(0, 120)}\n`
                      );
                    } catch (e2) {}
                  }
                  // Skip all-zero keystream packets; genuine handshake
                  // commands are 'k' (0x6b) and 'T' (0x54).
                  const allZero = /^0*$/.test(hex);
                  const cmd = parseInt(hex.slice(0, 2), 16);
                  if (!allZero && !Number.isNaN(cmd) && capturedOut.length < 8 &&
                      (cmd === 0x6b || cmd === 0x54 || capturedOut.length >= 2)) {
                    capturedOut.push(hex);
                  }
                  const haveK = capturedOut.some((p) => parseInt(p.slice(0, 2), 16) === 0x6b);
                  const haveT = capturedOut.some((p) => parseInt(p.slice(0, 2), 16) === 0x54);
                  if (((haveK && haveT) || (capturedScanned >= 300 && capturedOut.length)) && !capturedSent) {
                    capturedSent = true;
                    sendParent({
                      type: 'out_packets',
                      hash: config.hash || '',
                      packets: capturedOut.slice(0, 8)
                    });
                  }
                }
              } catch (e) {}
